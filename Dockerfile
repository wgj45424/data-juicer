...
# 替换为国内debian源，加速安装（修正换行语法）
RUN sed -i 's/deb.debian.org/mirrors.aliyun.com/g' /etc/apt/sources.list && \
    sed -i 's/security.debian.org/mirrors.aliyun.com/g' /etc/apt/sources.list

# 仅安装CPU编译依赖（修复&&换行语法，核心解决报错）
RUN apt-get update && \
...
RUN pip install --no-cache-dir \
    ...
    langdetect>=1.0.9 && \
    # 明确排除GPU相关依赖（修正语法）
    pip install --no-cache-dir --prefix=/tmp/python --no-deps torch cpuonly && \
    pip install --no-cache-dir --prefix=/tmp/python --no-deps tensorflow-cpu
